...
class ModelDevelopment:
    ...
    def load_data(self):
        ...
        try:
            ...
            # Load the Excel file
            self.process_data = pd.read_excel(
                data_path, sheet_name='Process Data')
            self.initial_conditions = pd.read_excel(
                data_path, sheet_name='Initial Conditions')
            self.process_kpis = pd.read_excel(
                data_path, sheet_name='Process KPIs')
    ...
    def prepare_data(self, scale, target):
        ...
        print(
            f"After forward fill - checking Batch ID presence: {'Batch ID' in filled_data.columns}")
        ...
        batch_stats = filled_data.groupby('Batch ID').apply(
            compute_batch_stats).reset_index()
        print(
        ...
        merged_data = pd.merge(batch_stats, ic_subset,
        ...
        merged_data = pd.merge(
            merged_data, kpi_subset[['Batch ID', target_col]], on='Batch ID', how='inner')
    ...
    def train_outlier_detection_models(self, X, scale, target):
        ...
        # Using dummy Y for unsupervised
        opls.fit(X_scaled, np.zeros(X_scaled.shape[0]))
    ...
    def train_models(self, scale, target):
        ...
        for cat_col in categorical_features:
            le = LabelEncoder()
            X_encoded[cat_col] = le.fit_transform(
                X_encoded[cat_col].astype(str))
        ...
        self.outlier_models[(scale, target)] = self.train_outlier_detection_models(
            X_final, scale, target)
        ...
        explained_variance = np.sum(best_pls.explained_variance_ratio_) if hasattr(
            best_pls, 'explained_variance_ratio_') else None
